...
export async function registerRoutes(app: Express, supabase: SupabaseClient): Promise<Server> {
  ...
  app.patch("/api/fitness-assessments/:id/override", async (req: any, res: Response) => {
    ...
    try {
      ...
      if (!assessment || assessment.userId !== userId) {
        return res.status(403).json({ error: "Assessment does not belong to user" });
      }
  ...
  app.post("/api/exercises/seed", async (req: any, res: Response) => {
    try {
      ...
      if (!equipmentList.includes("bodyweight")) {/* Lines 1115-1116 omitted */}
      if (equipmentList.length === 0 || (equipmentList.length === 1 && equipmentList[0] === "bodyweight")) {/* Lines 1119-1120 omitted */}
      ...
      if (error instanceof Error && error.message.includes("API key")) {/* Lines 1134-1135 omitted */}
