...
- **Comprehensive Fitness Assessment System**: Onboarding supports Bodyweight Test (including pike push-ups for shoulder strength), Weights Test, or an option to skip. Assessment data maps to 10 independent movement patterns for precise difficulty filtering: horizontal_push (chest), vertical_push (shoulders), pull, squat, lunge, hinge, core, rotation, carry, and cardio. Collects `daysPerWeek` (strictly 3, 4, or 5 days only), `selectedDays`, and comprehensive equipment selection (17 options). Session duration options: 30, 45, 60, or 90 minutes.
- **Template-Based Adaptive Training System**:
  ...
  - **Week-Level Program Planning**: Plans entire week's movement pattern distribution before selecting exercises, ensuring different primary patterns on different days for proper recovery and variety. 3-day: Pull+Hinge → Horizontal Push+Squat → Vertical Push+Pull Mix. 4-day: Pull+Horizontal Push → Lower → Vertical Push+Pull → Lower alternating emphasis. 5-day: Horizontal Push → Pull → Legs → Vertical Push+Pull → Lower classic split. Exercise tracking persists across entire week to prevent duplicates and maximize variety, with both horizontal push (chest) and vertical push (shoulders) appearing each week.
  ...
  - **Category-Specific Difficulty Filtering**: Enables independent progression across movement patterns (horizontal_push, vertical_push, pull, squat, lunge, hinge, cardio, core, rotation, carry).
