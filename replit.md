...
- **Template-Based Adaptive Training System**:
  ...
  - **CNS-Ordered Workout Progression**: Professional programming structure follows Central Nervous System (CNS) demand hierarchy: warmup → power → compounds → isolations → core → cardio finisher. 
    ...
    - **Metadata-Based Reordering**: Exercises tagged with source liftType and movement pattern during selection to prevent misclassification from name variations (e.g., "Plank [Alt]" retains core/isolation metadata).
    ...
    - **Database Classification Fix**: Core, rotation, and carry exercises correctly classified as liftType='isolation' (36 exercises updated from incorrect 'compound' default).
