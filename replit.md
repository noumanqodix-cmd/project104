...
- **Master Exercise Database (October 2025)**: A streamlined database of 95 exercises (consolidated from 163) categorized by equipment and movement pattern. Database now includes:
  - **Multi-Equipment Exercise Support**: 20 exercises support multiple equipment types (e.g., Deadlift works with bodyweight, barbell, dumbbells, kettlebell, resistance bands). AI selects specific equipment variant based on user's available equipment.
  - **Equipment Coverage**: 16 equipment types including cable machine (newly added: October 2025), dumbbells, barbell, kettlebell, resistance bands, pull-up bar, TRX, medicine ball, box, jump rope, rower, bike, treadmill, elliptical, assault bike, stair climber, and bodyweight. Removed foam roller and yoga mat equipment types (October 2025 consolidation).
  - **Equipment Field in Program Exercises**: AI-generated programs now include specific `equipment` field for each exercise, enabling precise exercise variant selection (e.g., "Barbell Deadlift" vs "Dumbbell Deadlift") from multi-equipment exercises
